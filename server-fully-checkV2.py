...
# ==========================================================
# MEMORY-SAFE CONFIG FOR 4GB RAM + 5M DOMAINS
# ==========================================================

TIMEOUT = 1
CONCURRENCY = 1000      # Conservative: 1000 concurrent connections
BATCH_SIZE = 100        # Small batches to save RAM
WRITE_BUFFER = 100      # Frequent disk writes
...
def iter_input_lines(path: Path) -> Iterator[str]:
    """Memory-efficient line iterator."""
...
async def check_port(semaphore: asyncio.Semaphore, domain: str, template: str, port: int, ssl: int):
...
async def check_domain(semaphore: asyncio.Semaphore, domain: str):
    tasks = []
...
async def process_streaming(files: List[Path], smtp_file: str, imap_file: str, pop3_file: str):
    semaphore = asyncio.Semaphore(CONCURRENCY)
    
    smtp_writer = SimpleCSVWriter(smtp_file)
    imap_writer = SimpleCSVWriter(imap_file)
    pop3_writer = SimpleCSVWriter(pop3_file)
    ...
    for current_file in files:
        ...
        for line in iter_input_lines(current_file):
            ...
            # NO seen_domains set - assumes pre-deduplicated input
            valid_domains += 1
            batch.append(domain)
            
            if len(batch) >= BATCH_SIZE:
                await _process_batch(batch, semaphore, smtp_writer, imap_writer, pop3_writer)
                checked += len(batch)
                batch = []
                
                # Progress update every 1000 domains
                if checked % 1000 == 0:
                    print(f"Progress: {checked:,} domains checked | "
                          f"SMTP: {smtp_writer.total:,} | "
                          f"IMAP: {imap_writer.total:,} | "
                          f"POP3: {pop3_writer.total:,}", end="\r")
    ...
    print(f"\n\nDone! Checked {checked:,} domains.")
    print(f"SMTP: {smtp_writer.total:,} | IMAP: {imap_writer.total:,} | POP3: {pop3_writer.total:,}")

async def _process_batch(domains, semaphore, smtp_w, imap_w, pop3_w):
    tasks = [check_domain(semaphore, d) for d in domains]
    results = await asyncio.gather(*tasks)
...
def main():
    ...
    if input_path.is_file():
        ...
        files = sorted(p for p in input_path.iterdir() if p.is_file() and p.suffix.lower() in {".txt", ".log", ".csv", ".xlsx"})
    else:
        print("File not found")
        return
    
    smtp_out = get_unique_output(SMTP_OUTPUT)
    imap_out = get_unique_output(IMAP_OUTPUT)
    pop3_out = get_unique_output(POP3_OUTPUT)
    
    # Headers
    for f in [smtp_out, imap_out, pop3_out]:
        with open(f, "w", encoding="utf-8", newline="") as fh:
            fh.write("domain,server,port,ssl\n")
    
    print(f"Processing {len(files)} file(s)...")
    print(f"Output: SMTP={smtp_out}, IMAP={imap_out}, POP3={pop3_out}")
    print("WARNING: With 5M domains, this will take several days!")
    print("Press Ctrl+C to resume later (results saved continuously)\n")
    
    try:
        asyncio.run(process_streaming(files, smtp_out, imap_out, pop3_out))
